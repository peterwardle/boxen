class people::peterwardle::applications::sublime {
    ...
    exec { 'Set Sublime Text Icon':
        ...
        cwd     => '/tmp',
        require => [Package['sublime-text3'], Exec['Create Sublime Text Icon Resource']],
    }

    exec { 'Subl: Package Control':
        creates => "${sublime_lib_directory}/Installed Packages/Package Control.sublime-package",
        command => "curl -sLo '${sublime_lib_directory}/Installed Packages/Package Control.sublime-package' 'https://packagecontrol.io/Package%20Control.sublime-package' &> /dev/null",
        require => Package['sublime-text3'],
    }
